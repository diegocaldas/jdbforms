...
<xsl:variable name="fileName"><xsl:value-of select="$tableName"/><xsl:text disable-output-escaping="yes">_partial_list_editable.jsp</xsl:text></xsl:variable>
...
<html>
    <head>
		<db:base />
		<title>Partial list editable --- file: <xsl:value-of select="$fileName"/></title>
