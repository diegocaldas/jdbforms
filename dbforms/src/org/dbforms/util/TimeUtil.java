...
public class TimeUtil {
	private static Category logCat =
		Category.getInstance(TimeUtil.class.getName());
	...
	public static final String seconds2String(long seconds) {
		...
		if (d > 0) {
			Object[] o =
				{ new Long(d), new Long(h), new Long(m), new Long(seconds)};
	...
	private static Calendar saveParseDate(
	...
	private synchronized static long saveParseTime(
		final SimpleDateFormat format,
		String formatString,
		String s) {
		Date d = null;
	...
	private static String reISO8601 =
		"(\\d\\d\\d\\d)(-(\\d\\d)(-(\\d\\d))?)?"
			+ "([T| ]?"
			+ "(\\d\\d):(\\d\\d)(:((\\d\\d)(\\.(\\d+))?)?)?"
			+ "(Z|([+-]\\d\\d:\\d\\d)|([A-Z]{3}))?)?";
