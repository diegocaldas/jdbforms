...
public class DbFormsValidatorUtil {
	static Category logCat = Category.getInstance(DbFormsValidatorUtil.class.getName());
	...
	* @return        <code>StringBuffer</code> : The complete javascript validation section to include in response.
	...
	public static StringBuffer getJavascript(
		...
		for (Iterator formIterator = formNames.iterator(); formIterator.hasNext();) {
							...
							while (enum.hasMoreElements()) {
								...
								if (field.getKey().equals(val)) {
									...
									if (fieldName.indexOf("insroot") != -1) {
										...
										results.append("\t    if(").append(ValidatorConstants.JS_UPDATE_VALIDATION_MODE).append(
											"==false) ");
									} else {
									...
									for (Iterator iVars = hVars.keySet().iterator(); iVars.hasNext();) {
										...
										if (Var.JSTYPE_INT.equalsIgnoreCase(jsType)) {
											...
											results.append(
												"this." + varKey + "=/" + ValidatorUtil.replace(varValue, "\\", "\\\\") + "/; ");
