 ...
 * session : prefix session__xxx . Uses  pageContext.getSession().findAttribute(xxx)
 ...
 * usual '.' notation, use '__' instead. The double _ is used since '.' notation
 * would not work with JasperReports
 * 
 * ex. &lt? session.putAttribute("adate". new Date()); ?&gt
 * session__adate__month , will be accessed as adate.getMonth();
 * @author Neal Katz
 */
public class JRDataSourceRSV implements JRDataSource,IGetSession {
	...
	private static Category logCat =
		Category.getInstance(JRDataSourceRSV.class.getName());
	...
	/**
		...
		*/
	public JRDataSourceRSV(ResultSetVector rsv, PageContext pageContext) {
		this.rsv = rsv;
	...
	public Object getFieldValue(JRField field) throws JRException {
		...
		if (search.startsWith("context__")) {
			...
			if (o == null) {
				logCat.debug(
					"Field not found in dbforms-config, trying field renamed to uppercase: "
						+ search.toUpperCase());
			...
			if (o == null) {
				logCat.debug(
					"Field not found in dbforms-config, trying field renamed to lowercase: "
						+ search.toLowerCase());
