...
/**
 * Use an iterator as data source
 * 
 * HOWTO
 * 1) create a Collection of X. Where X is a complex object with 'fields', X can either be
 * a Set or a Bean. The container must support Iterator(). Object X must be supported 
 * by apache bean utils
 * 2) somewhere in pagecontext (like session) put the Collection with key "jasper.input"
 * 3) invoke
 * 
 * gets field data from multiple sources, not just DBform fields
 * context : prefix context__xxx  . Uses pageContext.findAttribute(xxx)
 * session : prefix session__xxxx . Uses  pageContext.getSession().findAttribute(xxx)
 * internal : prefix internal_zzz
 * attribute of iterator object : no prefix  
 * 
 * For internal, only internal__rownum is currently defined
 * For content and session, xxxx may be a 'simple' type or 'complex' type
 * a 'complex' type is an object supporting javabean access, instead of the
 * usual '.' notation, use '__' instead.
 * 
 * @author Neal Katz
 */
...
public class JRDataSourceIter implements JRDataSource, IGetSession {
	...
	private static Category logCat =
		Category.getInstance(JRDataSourceIter.class.getName());
	...
	/**
		...
		* @param pageContext
		*/
	public JRDataSourceIter(Iterator iter, PageContext pageContext) {
	...
	public Object getFieldValue(JRField field) throws JRException {
		...
		if (search.startsWith("context__")) {
			logCat.debug(
				"Trying to find data for Page Context value: " + search);
	...
	private Object getPageContextValue(String search) {
		...
		try {
			...
			if (pos == -1) {
				...
				logCat.debug(
					"calling PropertyUtils.getProperty " + sa[0] + " " + sa[1]);
	...
	private Object getCurrentValue(String search) {
		...
		try {
			if (current != null) {
				...
				sa.replaceAll("__", ".");
				logCat.debug(
					"calling PropertyUtils.getProperty " + current + " " + sa);
