		...
		if(embeddedData==null) { // no embedded data is nested in this tag
			...
			*/
			...
			if((ce = this.getCustomEntry()) != null && ce.trim().length()>0)
			{
				...
				String aKey = org.dbforms.util.ParseUtil.getEmbeddedStringWithoutDots(ce,0,',');
				String aValue = org.dbforms.util.ParseUtil.getEmbeddedStringWithoutDots(ce,1,',');	
				...
				{
					isSelected = "true".equals(org.dbforms.util.ParseUtil.getEmbeddedStringWithoutDots(ce,2,','));
				}
				
				if(isSelected) selectedOptions.append("-").append(aKey);
