...
{                                                                
   ...
   private void init()
   {
      ...
      state.value   = ParseUtil.getParameter(
   ...
   private void renderSelectElement(StringBuffer buf)
   {
                ...
                && (state.customEntry.trim().length() > 0))
      {
         ...
         {
            isSelected = "true".equals(ParseUtil.getEmbeddedStringWithoutDots(
                                                state.customEntry, 2, ','));
         }
      ...
      {
         KeyValuePair aKeyValuePair = (KeyValuePair) state.embeddedData.elementAt(i);
