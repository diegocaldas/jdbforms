...
public class DbFormTag extends BodyTagSupport {
  ...
  static Category logCat = Category.getInstance(DbFormTag.class.getName()); // logging category for this class
  ...
  private int count;						// count (multiplicity, view-mode) of this form (n || -1), whereby n E N (==1,2..z)
  ...
  private int currentCount = 0;						// holds information about how many times the body of this tag has been rendered
  private String positionPath, positionPathCore;  // important in subforms: 5th row of subform in 2nd row of main form has path: "5@2"; in mainforms
  private boolean footerReached = false;	// if rendering of header and body of the form is completed and only the footer needs to be rendered yet

  private String multipart; // is either "true" or "false"
  private boolean _isMultipart = false; // multipart must be set to "true" if the form contains file-upload tags or if the form may be invoked by a form containing file-upload tags

  private String followUp;			// site to be invoked after action - nota bene: this followUp may be overruled by "followUp"-attributes of actionButtons
  private String target;				// pedant to the html-target attribute in html-form tag: the target frame to jump to
  private String autoUpdate = "false";	// if "true", at every action (navigation, insert, update, etc.) all input fields of ALL currently rendered rowsets are parsed and updated. many rows may be affected.
  ...
  private String parentField;		// used in sub-form:  field(s) in the main form that is/are linked to this form
  private String childField; 		// used in sub-form:  field(s) in this forme that is/are linked to the parent form
  private FieldValue[] childFieldValues; // used in sub-form: this data structure holds the linked childfield(s) and their current values it/they derive from the main form
...
/*
   ...
   public void appendToChildElementOutput(String str) {
	...
	public void setPageContext(PageContext pc) {
		...
		if(con == null) {
			...
			if(con == null)
			  throw new IllegalArgumentException("JDBC-Troubles: was not able to create connection, using the following DbConnection:"+aDbConnection.toString());

			pc.setAttribute("connection", con, PageContext.REQUEST_SCOPE);
		} else {
			logCat.info("connection found");	//#checkme - what about some connection validation??
		}
	}
  ...
  public int doStartTag() {
  	...
  	try {
			...
			tagBuf.append(">"); // supports RFC 1867 - multipart upload, if some database-fields represent filedata


			 if(tableName == null) { // if form is an emptyform -> we've fineshed yet - cancel all further activities!
			  ...
			  positionPathCore = "root";
				...
				initChildFieldValues();  // determine the value(s) of the linked field(s)
				if(childFieldValues == null) return SKIP_BODY;
			}
			...
			tagBuf.append("<input type=\"hidden\" name=\"source\" value=\""+request.getRequestURI()+"\">");
			...
			boolean useDefaultOrder = true; // tells us which definintion of field order (global or local) to use

			FieldValue[] orderConstraint;
			Vector orderFields;
			if(this.overrulingOrder != null) {  // if developer provided orderBy - Attribute in <db:dbform> - tag
				orderConstraint = overrulingOrder;
				orderFields = overrulingOrderFields;
				useDefaultOrder = false;
				logCat.info("using OverrulingOrder");
			} else {   // if developer provided orderBy - Attribute globally in dbforms-config.xml - tag
				...
				for(int i=0; i<tmpOrderConstraint.length; i++) {
					orderConstraint[i] = (FieldValue) tmpOrderConstraint[i].clone(); // cloning is necessary to keep things thread-safe! (we manipulate some fields in this structure.)
				}
	  ...
	  // an orderBY - clause is a MUST. we can't query well without it.
  		...
  		//String position = pageContext.getRequest().getParameter("pos_"+tableId);
			...
			if(firstPosition == null) firstPosition = lastPosition;
			...
			if(childFieldValues!=null && firstPosition!=null) {
				if(!checkLinkage(childFieldValues, firstPosition)) { // checking one of the 2 strings is sufficient
			...
			*/
			...
			if(webEvent!=null && webEvent instanceof NavigationEvent) {
				...
				// if we have no navigation event for this table
					...
					resultSetVector = table.doConstrainedSelect(table.getFields(), mergedFieldValues, orderConstraint, (firstPosition==null || count==0) ? FieldValue.COMPARE_NONE : FieldValue.COMPARE_INCLUSIVE , count, con); // if there is a navigation event but not for this table, then just navigate to a position (if it evists) or just select all data (if no pos or if endless form)
				}
			...
			if ( (webEvent==null || !(webEvent instanceof NavigationEvent) ) && (gotoHt==null || gotoHt.size()==0)   ) {
				...
				if(firstPosition!=null && (overrulingOrder==null || webEvent instanceof DatabaseEvent || webEvent instanceof NoopEvent   || webEvent instanceof EmptyEvent)) { // we have someting to look for, and we have _no_ change of search criteria #fixme - explain better

					logCat.info("§§§ B/I §§§");
					table.fillWithValues(orderConstraint, firstPosition);
					resultSetVector = table.doConstrainedSelect(table.getFields(), mergedFieldValues, orderConstraint, (count==0) ? FieldValue.COMPARE_NONE : FieldValue.COMPARE_INCLUSIVE, count, con); // in endless forms we never need to to compares

					logCat.info("we had someting to look for:"+firstPosition);
				} else { // we have no navigation objekt and no position we are supposed to go. so let's go to the first row.
					...
					if(count!=0 && webEvent!=null && webEvent instanceof InsertEvent && errors!=null && errors.size()>0) {
						...
						logCat.info("§§§ B/II/2 §§§");
						resultSetVector = table.doConstrainedSelect(table.getFields(), mergedFieldValues, orderConstraint, (firstPosition==null) ? FieldValue.COMPARE_NONE : FieldValue.COMPARE_INCLUSIVE, count, con);
					}
   ...
   * it initializes the data array childFieldValues, which holds the linked childfield(s) and
	...
	public void initChildFieldValues() {
		...
		if(ResultSetVector.isEmptyOrNull(parentForm.getResultSetVector())) {
		 childFieldValues = null; // childFieldValues remains null
		 return;
	 	}
		...
		if(len == 0 || len != parentFieldNames.size()) return;

		// get parent-table
		// we need it in oder to fetch the actual values of the mapped fields
...
public void initFilterFieldValues() {
	...
	*/
	public FieldValue[] initSearchFieldValues() {
		...
		if(searchFieldNames==null || searchFieldNames.size()==0) return null;
		...
		for(int i=0; i<searchFieldNames.size(); i++) {
		  ...
		  String searchFieldName = (String) searchFieldNames.elementAt(i); //. i.e search_1_12

		  String aSearchFieldValue = ParseUtil.getParameter(request, searchFieldName); // ie. search_1_12 is mapped to "john"
		  if(aSearchFieldValue!=null && aSearchFieldValue.trim().length()>0) {
			  ...
			  int tableId = Integer.parseInt(searchFieldName.substring(firstUnderscore+1, secondUnderscore)); // is equal to tableid, off course
			  ...
			  int algorithm = ("weak".equals(aSearchAlgorithm)) ? FieldValue.SEARCH_ALGO_WEAK : FieldValue.SEARCH_ALGO_SHARP;
		...
		if(criteriaFieldCount == 0) return null;
	...
	*/
	private Hashtable getNamesHashtable(String core) {
		...
		while(enum.hasMoreElements()) {
		  ...
		  result.put( fieldName, sb.toString() );  // in PHP slang we would call that an "associative array" :=)
		}
