...
public class DbFormTag extends BodyTagSupport implements TryCatchFinally {
		...
		try {
				...
				try {
					...
					out.println(
						"Sorry, viewing data from table "
							+ table.getName()
							+ " would violate a condition.<BR><BR>"
							+ sqle.getMessage());
			...
			*/
				...
				if ((count != 0)
					...
					navEvent =
						navEventFactory.createGotoEvent(
							...
							request,
							config,
							((count == 0) || ("true".equals(getBypassNavigation())) ? null : firstPosition));
				}
			}
			...
			if (navEvent != null) {
				...
				resultSetVector =
					navEvent.processEvent(mergedFieldValues, orderConstraint, count, firstPosition, lastPosition, con, dbConnectionName);
	...
	private void initFilterFieldValues() {
		...
		for (int i = 0; i < len; i++) {
			...
			filterFieldValues[i] = new FieldValue(filterField, value, operator, isLogicalOR);
