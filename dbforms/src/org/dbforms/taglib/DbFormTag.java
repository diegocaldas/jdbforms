...
public class DbFormTag extends BodyTagSupport implements TryCatchFinally {
   ...
   */
   public String produceLinkedTags() {
      ...
      DbBaseHandlerFactory parentHandler = null;
      DbBaseInputTag parentTag = null; 
      DbBaseHandlerFactory handler = null;
      DbBaseInputTag tag = null; 
      try {
         parentHandler = new DbBaseHandlerFactory(this.pageContext, this.parentForm, DbTextFieldTag.class);
         parentTag = (DbBaseInputTag) parentHandler.getTag();
         handler = new DbBaseHandlerFactory(this.pageContext, this, DbTextFieldTag.class);
         tag = (DbBaseInputTag) handler.getTag();
      } catch (Exception e) {
         logCat.error("produceLinkedTags", e);
      }
      // childFieldValues may be null, if we have
      // a free form select using attribute whereClause
      if ((childFieldValues != null) && (parentTag != null) && (tag != null)) {
         for (int i = 0; i < childFieldValues.length; i++) {
            if (childFieldValues[i].getRenderHiddenHtmlTag()) {
               parentTag.setField(childFieldValues[i].getField());
               tag.setField(childFieldValues[i].getField());
               buf.append("<input type=\"hidden\" name=\"");
               buf.append(tag.getFormFieldName());
               buf.append("\" value=\"");
               buf.append(parentTag.getFormattedFieldValue());
               buf.append("\" />");
               buf.append(tag.getFormatInputField());
            }
         }
      }
      return buf.toString();
