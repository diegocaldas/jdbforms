...
public class DbFormTag extends BodyTagSupport implements TryCatchFinally
{
    ...
    */
   public boolean isCaptionResource()
   {
    ...
    */
   public boolean isJavascriptFieldsArray()
   {
    ...
    */
   public boolean isRedisplayFieldsOnError()
   {
    ...
    */
   public boolean isBypassNavigation()
   {
      ...
      try
      {
         ...
         if (parentForm == null)
         {
            ...
            if (!Util.isNull(validationFct) && !Util.isNull(getOnSubmit()))
            {
               ...
               {
                  cmds[cmds.length - 1] = cmds[cmds.length - 1].substring(
                                                   "return".length());
               }

               if (cmds[cmds.length - 1].endsWith(";"))
               {
                  cmds[cmds.length - 1] = cmds[cmds.length - 1].substring(0, 
                                                                          cmds[cmds.length - 1].length() - 1);
               }

               cmds[cmds.length - 1] = "return " + cmds[cmds.length - 1]
                                       + " && " + validationFct;
               s = "";
         ...
         if (navEvent == null)
         {
            ...
            {
               String myPosition = ((count == 0) || isBypassNavigation())
                                      ? null : firstPosition;
               navEvent = navEventFactory.createGotoEvent(table, request, 
                                                          config, myPosition);
            }
         ...
         if (!ResultSetVector.isNull(resultSetVector))
         {
            ...
            pageContext.setAttribute("position_" + tableName.replace('.', '_'), 
                                     Util.decode(table.getPositionString(
                                                          resultSetVector)));
         }
   ...
   public String getChildFieldValue(String name)
   {
      ...
      if ((webEvent != null)
                && (webEvent.getType() == EventType.EVENT_NAVIGATION_RELOAD))
      {
         ...
         String keyIndex = (getFooterReached())
                              ? (Constants.FIELDNAME_INSERTPREFIX
                                + getPositionPathCore()) : getPositionPath();
         StringBuffer buf = new StringBuffer();
