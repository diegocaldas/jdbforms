 ...
 * @author Joachim Peer
 */
public class DbFormTag extends TagSupportWithScriptHandler implements
		TryCatchFinally {
	/** logging category for this class */
	private static Category logCat = Category.getInstance(DbFormTag.class
			.getName());
	 ...
	 * For better performance. Else it's the webserver will generate it each
	 * time.
	 */
	...
	/**
	 * List of all child field name with assosciate generated name. Ex: "champ1" :
	 * "f_0_3@root_3"
	 */
	...
	/**
	 * Gets the positionPathCore attribute of the DbFormTag object. <br>
	 * Ie. "root", "123@35@root"
	 * 
	...
	/**
	 * Grunikiewicz.philip@hydro.qc.ca 2001-08-17
	 * 
	 * Added an attribute to allow developer to bypass navigation: If we are not
	 * going to use navigation, eliminate the creation of, and execution of
	 * "fancy!" queries.
	 * 
	 * 
	 * Henner.Kollmann@gmx.de 2002-07-03 Added onSubmit support
	 * Grunikiewicz.philip@hydro.qc.ca 2001-10-22
	 * 
	 ...
	 * 
	 * 
	 * Grunikiewicz.philip@hydro.qc.ca 2001-11-28
	 * 
	 * Introducing Free-form select queries. Using the whereClause attribute,
	 * developers are now able to specify the ending of an actual DbForms query.
	 * Dbforms generates the "Select From" part, and the developers have the
	 * responsibility of completing the query. ie: adding the whereClause, an
	 * orderBy, etc... Note that using this function renders navigation
	 * impossible!
	 * 
	 * @return Description of the Return Value
	 */
	public int doStartTag() {
		...
		try {
			...
			if (parentForm == null) {
				...
				if (hasJavascriptValidationSet()) {
					...
					validationFct = Character.toUpperCase(validationFct
							.charAt(0))
							+ validationFct
									.substring(1, validationFct.length());
			...
			tagBuf.append("<input type=\"hidden\" name=\"lang" + "\" value=\""
					+ locale.getLanguage() + "\"/>");
			tagBuf.append("<imput type=\"hidden\" name=\"country"
			 ...
			 * (NavigationEvents, including GotoEvent)
			 *  ) firstPos, lastPos: Strings containing key-fieldValues and
			 * indicating a line to go to if we have no other information. this
			 * may happen if a subform gets navigated. the parentForm is not
			 * involved in the operation but must be able to "navigate" to its
			 * new old position. [#checkme: risk of wrong interpreation if jsp
			 * calls jsp - compare source tags?]
			 *  ) mergedFieldValues: this is a cumulation of all rules which
			 * restrict the result set in any way. it is build of
			 ...
			 * customerID 100)
			 *  - filterFieldValues: if a filter is applied to the resultset we
			 * always need to select the _filtered_ resultset
			 *  - searchFieldValues: if a search is performed we just want to
			 * show fields belonging to the search result (naturally ;=)
			 ...
			 * describe above)
			 * 
			 *  ) count: this is a property of DbFormTag. Its relevance is that
			 * certain operations need to be performed differently if count==0,
			 * which means the form is an "endless form".
			 */
			...
			if (navEvent != null) {
				...
				resultSetVector = navEvent.processEvent(mergedFieldValues,
						...
						count, firstPosition, lastPosition, dbConnectionName,
						con);
			...
			DbFormContext dbContext = new DbFormContext(table
					.getNamesHashtable(Constants.FIELDNAME_SEARCH), table
					.getNamesHashtable(Constants.FIELDNAME_SEARCHMODE), table
					.getNamesHashtable(Constants.FIELDNAME_SEARCHALGO),
					resultSetVector);
			if (!ResultSetVector.isNull(resultSetVector)) {
				dbContext.setCurrentRow(resultSetVector.getCurrentRowAsMap());
				dbContext.setPosition(Util.encode(table
			...
			if (!ResultSetVector.isNull(resultSetVector)) {
				...
				pageContext.setAttribute("position_"
						+ tableName.replace('.', '_'), Util.encode(table
						.getPositionString(resultSetVector), pageContext
						.getRequest().getCharacterEncoding()));
			}
	 ...
	 *       <br/>Maybe we should parse all of them ....
	 */
	private FieldValue[] initSearchFieldValues() {
		...
		Vector searchFieldNames = ParseUtil.getParametersStartingWith(request,
				Constants.FIELDNAME_SEARCH + this.tableId);
		...
		for (int i = 0; i < searchFieldNames.size(); i++) {
			...
			if (!Util.isNull(aSearchFieldValue)) {
				...
				int secondUnderscore = searchFieldName.indexOf('_',
						firstUnderscore + 1);
				int tableId = Integer.parseInt(searchFieldName.substring(
				...
				if (f != null) {
					...
					if ((aSearchAlgorithm == null)
						...
						if (jump > 0) {
							aSearchFieldValue = aSearchFieldValue.substring(
									jump).trim();
						}
