...
public class DbFormTag extends BodyTagSupport implements TryCatchFinally {
	...
	private static Category logCat =
		Category.getInstance(DbFormTag.class.getName());
	...
	private static NavEventFactory navEventFactory =
		NavEventFactoryImpl.instance();
		...
		try {
			...
			if (parentForm == null) {
				...
				if ((this.getAction() != null)
					...
					tagBuf.append(
						response.encodeURL(
							request.getContextPath() + "/servlet/control"));
				}
			...
			tagBuf.append(
				"<input type=\"hidden\" name=\"invtable\" value=\""
					+ tableId
					+ "\"/>");
			...
			String requestSqlFilterString =
				DbFilterTag.getSqlFilter(request, this.getTable().getId());
			...
			if (!Util.isNull(requestSqlFilterString)) {
				sqlFilterParams =
					DbFilterTag.getSqlFilterParams(
						request,
						this.getTable().getId());
			}
			...
			// if developer provided orderBy - Attribute globally in dbforms-config.xml - tag
				...
				for (int i = 0; i < tmpOrderConstraint.length; i++) {
					orderConstraint[i] =
						(FieldValue) tmpOrderConstraint[i].clone();
				}
			...
			if (childFieldValues == null) {
				...
				mergedFieldValues =
					new FieldValue[childFieldValues.length
			...
			if (navEvent == null) {
				...
				if (!Util.isNull(gotoPrefix)) {
					...
					Vector v =
						ParseUtil.getParametersStartingWith(
							request,
							gotoPrefix);
					...
					for (int i = 0; i < v.size(); i++) {
						...
						String fieldName =
							paramName.substring(gotoPrefix.length());
						logCat.debug("fieldName=" + fieldName);

						String fieldValue =
							ParseUtil.getParameter(request, paramName);
		...
		try {
			...
			if (javascriptDistinctFunctions.size() > 0) {
				...
				while (enum.hasMoreElements()) {
					...
					StringBuffer sbFonction =
						(StringBuffer) javascriptDistinctFunctions.get(aKey);
	...
	private FieldValue[] initOverrulingOrder(HttpServletRequest request) {
		...
		FieldValue[] overrulingOrder =
			table.createOrderFieldValues(orderBy, request, false);
	...
	private void initChildFieldValues() {
		...
		String aPosition =
			parentForm.getTable().getPositionString(
	...
	private FieldValue[] initSearchFieldValues() {
		...
		for (int i = 0; i < searchFieldNames.size(); i++) {
			...
			String aSearchFieldPattern =
				ParseUtil.getParameter(request, Constants.FIELDNAME_PATTERNTAG  + searchFieldName);
			String aSearchFieldValue =
				ParseUtil.getParameter(request, searchFieldName);
			...
			if (!Util.isNull(aSearchFieldValue)) {
				...
				int secondUnderscore =
					searchFieldName.indexOf('_', firstUnderscore + 1);
				int tableId =
					Integer.parseInt(
				...
				int fieldId =
					Integer.parseInt(
						searchFieldName.substring(secondUnderscore + 1));
				...
				if ((aSearchAlgorithm == null)
					|| (aSearchAlgorithm.toLowerCase().indexOf("extended")
						== -1)) {
					...
					StringTokenizer st =
						new StringTokenizer(" " + aSearchFieldValue + " ", "-");
					int tokenCounter = 0;
					...
					while (st.hasMoreTokens()) {
						...
						if (aSearchFieldValue.length() > 0) {
							switch (tokenCounter) {
								case 1 :
									operator =
										Constants.FILTER_GREATER_THEN_EQUAL;
									break;

								case 2 :
									operator =
										Constants.FILTER_SMALLER_THEN_EQUAL;
									break;
					...
					if (jump > 0) {
						aSearchFieldValue =
							aSearchFieldValue.substring(jump).trim();
					}
			...
			while (enum.hasMoreElements()) {
				...
				if (arrValues.size() == 1) {
					result.append("\"").append(
						(String) arrValues.get(0)).append(
						"\"");
				} else {
					for (int i = 0; i <= (arrValues.size() - 1); i++) {
						result.append("\"").append(
							(String) arrValues.get(i)).append(
							"\"");
	...
	private StringBuffer generateJavascriptValidation() {
		if (isSubForm()) {
			...
			ValidatorResources vr =
				(ValidatorResources) pageContext
					.getServletContext()
					.getAttribute(
	...
	public String getChildFieldValue(String name) {
		...
		if ((webEvent != null)
			&& (webEvent.getType().equals(EventType.EVENT_NAVIGATION_RELOAD))) {
			...
			String keyIndex =
				(getFooterReached())
					? (Constants.FIELDNAME_INSERTPREFIX + getPositionPathCore())
					: getPositionPath();
			StringBuffer buf = new StringBuffer();
