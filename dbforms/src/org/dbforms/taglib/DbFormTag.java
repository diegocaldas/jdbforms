 ...
 * @author Joachim Peer
 */
public class DbFormTag extends TagSupportWithScriptHandler
    ...
    *
    * @param value The new multipart value
    */
    ...
    * object
    *
    * @param jsFctName The feature to be added to the JavascriptFunction
    *        attribute
    ...
    * Sometimes we use dbForms to simply display information on screen but
    * wish to call another servlet to process the post. I've added the
    * 'action' attribute to allow a developer to set the wanted action instead
    * of defaulting to the dbforms control servlet. Important: Use a fully
    ...
    * completing the query. ie: adding the whereClause, an orderBy, etc...
   ...
   public int doStartTag() {
      try {
         ...
         DbFormContext dbContext = new DbFormContext(table.getNamesHashtable(
         ...
         if (!ResultSetVector.isNull(resultSetVector)) {
            ...
            dbContext.setPosition(Util.encode(table.getPositionString(
                     resultSetVector),
                  pageContext.getRequest().getCharacterEncoding()));
         }
    ...
    * the position strings is valid in the current state it might be invalid
    * if the position of the parent form has been changed (by a navigation
    * event) (=> the position-strings of childforms arent valid anymore)
    *
   ...
   private StringBuffer generateJavascriptValidation() {
      if (isSubForm()) {
         ...
         ValidatorResources vr = (ValidatorResources) pageContext.getServletContext()
                                                                 .getAttribute(ValidatorConstants.VALIDATOR);
         DbFormsErrors      errors = (DbFormsErrors) pageContext.getServletContext()
