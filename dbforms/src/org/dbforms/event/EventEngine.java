 ...
 * out which Event(s) need to be instanciated. The fine-grained parsing (parsing
 * of additional data, etc) is done by the WebEvent-Object itself (in order to
 * hide complexity from this class and to keep the framework open for
...
public class EventEngine {
    ...
    private static Category logCat = Category.getInstance(EventEngine.class
            .getName());
    ...
    public WebEvent generatePrimaryEvent() {
        ...
        if (action.startsWith("re_")) {
            ...
            e = new PageReloadEvent(ParseUtil.getEmbeddedStringAsInteger(
                    action, 2, '_'), request, config);
        ...
        case EventType.EVENT_GROUP_NAVIGATION: {
            logCat
                    .info("::generatePrimaryEvent - generating a navigation event");
        ...
        default: {
            logCat
                    .error("::generatePrimaryEvent - WARNING: generating NO event. Why ?");
     ...
     * 
     * @param exclude
     *            the parent web event (related to the main form)
     * @return DOCUMENT ME!
     */
    public Enumeration generateSecundaryEvents(Table actTable, WebEvent exclude) {
        ...
        if (res.equalsIgnoreCase("true")
                || (res.equalsIgnoreCase("OnUpdateOnly") && exclude.getType()
                        .equals("update"))) {
            // we can only update existing rowsets. so we just look for
            // key-values
            String paramStub = "k_" + actTable.getId() + "_";
            Enumeration keysOfCurrentTable = ParseUtil
                    .getParametersStartingWith(request, paramStub).elements();
            ...
            while (keysOfCurrentTable.hasMoreElements()) {
                ...
                if (!collissionDanger || (excludeTableId != actTable.getId())
                    ...
                    DatabaseEvent e = dbEventFactory.createUpdateEvent(actTable.getId(),
            ...
            if (collissionDanger && (excludeTableId != actTable.getId())) {
                ...
                if (v.size() > 0) {
                    ...
                    DatabaseEvent e = dbEventFactory.createInsertEvent(actTable.getId(), keyId,
                            request, config);
                    result.addElement(e);
                }
            }
     ...
     * 
     * @param contextPath
     *            The new sourcePath value
     * @param sourcePath
     *            The new sourcePath value
     * @return Description of the Return Value
     */
    private String setSourcePath(String contextPath, String sourcePath) {
        if (!Util.isNull(contextPath) && !Util.isNull(sourcePath)
    ...
    private void setEventFollowUp(WebEvent e, String action) {
        ...
        // display.
        String followUp = ParseUtil.getParameter(request, "data" + action
                + "_fu");

        // if not...
        // ...then check if §2-followup exists (should always exist!)
        if (followUp == null) {
            followUp = ParseUtil.getParameter(request, "fu_"
                    + ((e == null) || (e.getTable() == null) ? -1 : e
                            .getTable().getId()));
        }
        ...
        String followUpOnError = ParseUtil.getParameter(request, "data"
                + action + "_fue");

        // if not...
        // ...then check if §2-followup exists
        if (followUpOnError == null) {
            followUpOnError = ParseUtil.getParameter(request, "fue_"
                    + ((e == null) || (e.getTable() == null) ? -1 : e
                            .getTable().getId()));
        }
