...
public class NavPrevEvent extends NavigationEvent {
	private static Category logCat = Category.getInstance(NavPrevEvent.class.getName()); // logging category for this class
	 ...
	 * @param  config the config object
	 */
	public NavPrevEvent(Table table, HttpServletRequest request, DbFormsConfig config) {
	...
	public ResultSetVector processEvent(	FieldValue[] childFieldValues,
			...
			ResultSetVector resultSetVector = table.doConstrainedSelect(table.getFields(), childFieldValues, orderConstraint, Constants.COMPARE_EXCLUSIVE, count, con);
			...
			if (resultSetVector.size() == 0) {
				...
				resultSetVector = table.doConstrainedSelect(table.getFields(), childFieldValues, orderConstraint, Constants.COMPARE_NONE, count, con);
			}
