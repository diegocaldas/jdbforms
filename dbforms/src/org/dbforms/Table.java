...
public class Table {
	...
	private Vector createOrderFVFromAttribute(String order) {
		...
		if (order != null) {
			...
			while (st.hasMoreTokens()) {
				...
				boolean sortDirection = Constants.ORDER_ASCENDING; // we propose the default
				...
				if (index != -1) // Do we have a command, if not assume ASC order
					...
					if (pos == -1) // ASC not found, try descending
						...
						if (index != -1) {
							sortDirection = Constants.ORDER_DESCENDING; // ... we set DESC.
						}
					}
				}
		...
		for (int i = 0; i < sortFields.size(); i++) {
			...
			if (sortState.equalsIgnoreCase("asc") || sortState.equalsIgnoreCase("desc")) {
				boolean sortDirection =
					sortState.equalsIgnoreCase("asc") ? Constants.ORDER_ASCENDING : Constants.ORDER_DESCENDING;
				FieldValue fv = new FieldValue();
	...
	public FieldValue[] mapChildFieldValues(
		...
		for (int i = 0; i < len; i++) {
			...
			childFieldValues[i] = new FieldValue(childField, currentParentFieldValue);
		}
