...
public class XMLConfigGenerator implements PropertyNames {
   ...
   public static String createXMLOutput(ProjectData projectData,
      ...
      if (projectData.getProperty(EXAMINE_TABLES)
                           .equalsIgnoreCase(TRUESTRING)) {
         typesVec.add("TABLE");
      }

      if (projectData.getProperty(EXAMINE_VIEWS)
                           .equalsIgnoreCase(TRUESTRING)) {
         typesVec.add("VIEW");
      }

      if (projectData.getProperty(EXAMINE_SYSTABS)
                           .equalsIgnoreCase(TRUESTRING)) {
      ...
      try {
         ...
         if (includeCatalog) {
            try {
               ...
               warningMessage.append("<li>Error reading catalog separator from database:   <br>"
                                     + ex.getMessage() + "<br>"
                                     + "  Using default '" + catalogSeparator
                                     + "' instead.");
            }
         }
         ...
         for (int i = 0; i < tableNames.size(); i++) {
            ...
            if (includeCatalog
               ...
               result.append(catalogName.trim())
   ...
   private static void addSingleReference(HashMap   hm,
                                          ResultSet rsk,
                                          Vector    knownTables,
                                          boolean   includeCatalog,
                                          String    catalogSeparator,
    ...
    * @return
    */
   private static String remapType(int    dbms,
                                   String typeName) {
      if (dbms == DBMS_MYSQL) {
