...
public class XMLConfigGenerator implements PropertyNames {
   ...
   static final String[] knownFieldTypes = { "tinyint", "int", "smallint", "integer", "bigint", // mapped to INTEGER
    ...
    * @return DOCUMENT ME!
    */
   public static HashMap getForeignKeyInformation(
   ...
   private static void addSingleReference(
      HashMap hm,
      ResultSet rsk,
      Vector knownTables,
      boolean includeCatalog,
   ...
   public static String getForeignKeyTags(HashMap hm, String catalog, String schema, String table) {
      ...
      while (forKeyIt.hasNext()) {
         ...
         sb
            ...
            .append("\"")
            .append("\n\t\t      foreignTable=\"")
            .append(fk.getForeignTable())
            .append("\"")
            .append("\n\t\t      displayType=\"")
            .append((v.size() == 1) ? "select" : "none")
            .append("\"")
            .append(">\n");
         ...
         for (int jj = 0; jj < v.size(); jj++) {
            ...
            sb
               .append("\t\t     <reference local=\"")
               .append(ref.getLocal())
               .append("\"")
               .append("\n\t\t              foreign=\"")
               .append(ref.getForeign())
               .append("\"/>\n");
         }
   ...
   public static String createXMLOutput(ProjectData projectData, boolean createGuiMessagewindow) throws Exception {
      ...
      boolean foreignKeyTryGetCrossReferences =
         USE_GETCROSSREFERENCES.equalsIgnoreCase(projectData.getProperty(FOREIGNKEY_DETECTION));
      ...
      String catalog =
         projectData.getProperty(CATALOG_SELECTION).equalsIgnoreCase(ALL) ? null : projectData.getProperty(CATALOG);

      String schemaPattern =
         projectData.getProperty(SCHEMA_SELECTION).equalsIgnoreCase(ALL) ? null : projectData.getProperty(SCHEMA);

      String tableNamePattern =
         projectData.getProperty(TABLE_SELECTION).equalsIgnoreCase(ALL) ? null : projectData.getProperty(TABLE_NAME_PATTERN);

      String dateFormatTag =
         projectData.getProperty(DATE_FORMAT).equalsIgnoreCase("")
            ? ""
      ...
      try {
         ...
         if ((!useAutoCommitMode) && (!dbmd.supportsTransactions())) {
            showWarning = true;
            warningMessage.append(
               "<li>Transaction mode not supported by DBMS, connection is <br>" + "    automatically set to autocommit mode.");
         ...
         for (int i = 0; i < tableNames.size(); i++) {
            ...
            if (checkForAutoIncFields) {
               ...
               if (pos >= 0) {
                  sqlStmtPS =
                     autoIncColumnsQuery.substring(0, pos)
                        + tableName
               ...
               if (pos >= 0) {
                  sqlStmtPS =
                     sqlStmtPS.substring(0, pos) + catalogName + sqlStmtPS.substring(pos + catalogPlaceholder.length());
               }
            ...
            while (rsFields.next()) {
               ...
               if (generateDefaultValues) {
                  defaultValue = rsFields.getString(13);
                  // don't bother keeping defaultValue if its "" or "0"
               ...
               // programaticly change type for some special caes
               typeName = remapType(dbms, typeName);
         ...
         result.append("\t<dbconnection\n");
         result.append("\t\tname   = \"" + xmlClean(jdbcURL) + "\"\n");
