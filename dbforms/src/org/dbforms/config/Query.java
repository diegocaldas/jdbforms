...
/**
 * <p>
 * This class represents the query tag in dbforms-config.xml (dbforms config xml
 * file)
 * </p>
 * 
 * <p>
...
public class Query extends Table {
	/** log4j category */
	private static Category logCat = Category
			.getInstance(Query.class.getName());
	...
	public int populateWhereEqualsClause(FieldValue[] fvEqual,
		...
		curCol = super.populateWhereEqualsClause(getFieldValueWhere(fvEqual),
				ps, curCol);
	 ...
	 * key-fields) overloaded from Table Specials: if key of view is not defined
	 * (if view has not defined fields) use keys from parent table
	 * 
	 * @return the keys
	 */
	 ...
	 * 
	 * @param followAfterWhere
	 *            The followAfterWhere to set
	 */
