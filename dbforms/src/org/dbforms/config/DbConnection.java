...
public class DbConnection implements javax.sql.DataSource {
   ...
   /** ContextDS flag. If set, an existing DataSource from Context-Scope would be used (e.g. from Struts) */
   ...
   /**
   * sets the name of the parameter from context-Scope which 
   ...
   */
   ...
   /**
   * returns the name of the parameter from context-Scope which 
   * contains a DataSource-Object
   *
   ...
   */
   ...
   public String toString() {
      ...
      if (pow2) {
         buf.append(
            ", connectionProviderClass=" + connectionProviderClass).append(
            ", connectionPoolURL=" + connectionPoolURL);
      }
      ...
      try {
         ...
         if (ctx != null) {
            ...
            if (ds != null) {
               ...
               logCat.error(
                  "::getConnectionFromJNDI - DataSource object is null");
            }
         } else {
            logCat.error(
               "::getConnectionFromJNDI - no context object avaiable");
         }
      } catch (NamingException ne) {
         logCat.error(
            "::getConnectionFromJNDI - cannot retrieve a connection from JNDI:",
            ne);
      } catch (Exception e) {
   ...
   private Connection getConnectionFromFactory() {
      ...
      try {
         ...
         logCat.error(
            "::getConnectionFromFactory - cannot retrieve a connection from the connectionFactory",
            se);
      }
   ...
   private Connection getConnectionFromDriverManager() {
      ...
      try {
         ...
         logCat.error(
            "::getConnectionFromDriverManager - cannot retrieve a connection from DriverManager",
            e);
      }
