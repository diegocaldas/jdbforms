			...
			if (escaper == null) {
				if (getTable() == null) {
					try {
						...
						logCat
								.error("cannot create the new default escaper",
										e);
					}
				} else {
	...
	public Format getFormat(String pattern, Locale locale) {
		...
		if (!Util.isNull(pattern)) {
			if ("short".startsWith(pattern.toLowerCase())) {
				...
				pattern = MessageResourcesInternal.getMessage(
						"dbforms.pattern." + getFieldType() + "." + pattern,
						locale);
