 ...
 * interceptor interface Allows developers to define more then one error - to
 * be used in conjunction with the XML errors mechanism
 */
public class MultipleValidationException extends ValidationException {
    ...
    * Override the <code>getMessage</code> method of the
    * <code>Throwable</code> class. <br>
    * Note: the Throwable class' <code>toString</code> method calls
    * <code>getLocalizedMessage()</code> to get the string representation of
    * the detaile error message. The original
    * <code>getLocalizedMessage()</code> implementation calls
    * <code>getMessage()</code>. <br>
    * Overriding <code>getMessage</code> to get the multiple messages from
