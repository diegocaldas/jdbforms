...
public class FieldValue implements Cloneable, Serializable {
	...
	public static FieldValue createFieldValueForSearching(Field field,
			String fieldValue, Locale locale, int operator, int searchMode,
			int searchAlgorithm, boolean logicalOR) {
	 ...
	 * 
	 * @param newLogicalOR
	 *            the logicalOR attribute value to set
	 */
	 ...
	 * 
	 * @param string
	 *            the oldValue for this Field object
	 */
	 ...
	 *            The new renderHiddenHtmlTag value
	 */
	...
	private Date parseDATE(String value) {
		...
		if (getLocale() != null) {
			try {
				SimpleDateFormat f = (SimpleDateFormat) getField().getFormat(
		...
		try {
			// 20031123-HKK: patch suggested by Igor Longagnani
			// [i.longagnani@tiscali.it]:
	...
	private Double parseDOUBLE(String value) {
		...
		if (getLocale() != null) {
			try {
				DecimalFormat f = (DecimalFormat) getField().getFormat(
						getPattern(), getLocale());
