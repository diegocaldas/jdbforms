...
public class StartReportServlet extends HttpServlet {
	...
	private void process(HttpServletRequest request, HttpServletResponse response) {
		// create report name
		try {
			String reportFile = getReportFileFullName(request.getPathInfo(), getServletContext(), request,response);
			JRDataSource dataSource = getDataFromForm(getServletContext(), request, response);
	 ...
	 *
	 */
	public static void process(String reportFileFullName, JRDataSource dataSource, ServletContext context, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			try {
				...
				JasperPrint jPrint;
				if (dataSource == null)
					jPrint = JasperFillManager.fillReport(reportFileFullName + ".jasper", map, repParam.getConnection());
				else
					jPrint = JasperFillManager.fillReport(reportFileFullName + ".jasper", map, dataSource);
	...
	public static String getReportFileFullName(String reportFileName, ServletContext context, HttpServletRequest request, HttpServletResponse response) {
	...
	public static String getConnectionName(HttpServletRequest request) {
		...
		if ((webEvent != null) && (webEvent.getTableId() != -1)) {
			res = ParseUtil.getParameter(request, "invname_" + webEvent.getTableId(), res);
		}
		return res;
	}
	...
	public static void handleNoData(HttpServletRequest request, HttpServletResponse response) {
		sendErrorMessage(request,response,"no data found to print");
	}

	private static void sendErrorMessage(HttpServletRequest request, HttpServletResponse response, String message) {
		try {	
	...
	private static void handleNoReport(HttpServletRequest request, HttpServletResponse response) {
		sendErrorMessage(request, response,"report " + request.getPathInfo() + " not found");
	}


	private static void handleEmptyResponse(HttpServletRequest request, HttpServletResponse response) {
		sendErrorMessage(request, response, "JasperReport Error<BR>No output generated");
	}


	private static byte[] exportToPDF(JasperPrint jasperPrint) throws JRException {
	...
	private static byte[] exportToXLS(JasperPrint jasperPrint) throws JRException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
    ...
    private static void compileJasper(ServletContext context, String reportFile) throws JRException {
	...
	private static void checkIfNeedToCompile(ServletContext context, String reportFile) throws JRException {
	...
	private JRDataSource getDataFromForm(ServletContext context, HttpServletRequest request, HttpServletResponse response) {
