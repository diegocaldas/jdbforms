...
/**
 * Connection provider for Protomatter Connection pool. <br>
 * See <code>http://protomatter.sourceforge.net/</code> for further
 * informations.
 *
 * @author Luca Fossato
 */
public class ProtomatterConnectionProvider extends ConnectionProvider {
   ...
   /**
    * the max pool size (default is -1). If the max pool size is -1, the pool
    * grows infinitely.
    */
   protected static final String CP_PROPS_MAXSIZE = "pool.maxSize";

   /**
    * the grow size (default is 1). When a new object is needed, this many are
    * created.
    */
    ...
    * This is useful for database connection pools where it's possible to
    * overload the database by trying to make too many connections too
    * quickly.
    */
   protected static final String CP_PROPS_CREATEWAITTIME = "pool.createWaitTime";

   /**
    * A SQL statement that is guaranteed to return at least 1 row. For Oracle,
    * this is "select 1 from dual" and for Sybase it is "select 1". This
    * statement is used as a means of checking that a connection is indeed
    * working.
    */
   protected static final String CP_PROPS_VALIDITYCHECKSTATEMENT = "jdbc.validityCheckStatement";

   /**
    * If this property is present, and the
    * <code>pool.maidThreadCheckInterval</code> property is also present, then
    * a thread will be created that looks for connections that have been idle
    * for more than <code>pool.maxConnectionIdleTime</code> seconds. When this
   ...
   /**
    * this is the number of seconds between attempts by the maid thread (if
    * present) to find idle connections.
    */
   ...
   protected void init() throws Exception {
      ...
      if ((props = getPrefs()
         ...
         for (Enumeration e = props.propertyNames(); e.hasMoreElements();) {
            ...
            logCat.info("::init - JDBC property [" + key + "] = ["
    ...
    * @param key the key value used to retrieve the value from the props
    *        object. The same key is used to store the retrieved value into
    *        the args hashMap
    * @param defValue the default value to use if the input key does not
    *        retrieve a valid value from the props object
    */
   ...
   private final void setIntegerArg(Hashtable  args,
      ...
      if (!Util.isNull(value)) {
         ...
         logCat.info("::setIntegerArg - [" + key + "] = [" + value + "]");
      }
   }
}
