...
<xsl:variable name="fileName"><xsl:value-of select="$tableName"/><xsl:text disable-output-escaping="yes">_list_and_single.jsp</xsl:text></xsl:variable>
...
<html>
	<head>
		<db:base/>
		<title>List and Single --- file: <xsl:value-of select="$fileName"/></title>
...
<db:header>
	...
	<tr align="left" valign="middle" class="clsHeaderDataTableRow">
		<td><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></td>
		<xsl:for-each select="field[not(@autoInc) or @autoInc='false']">
			<td>
...
<xsl:if test="not($maxRows='*')">
	...
	<table align="center" border="0">
		<tr valign="middle"><hr width="400"/></tr>
		<tr>
			<td align="right"><db:updateButton caption="Update" associatedRadio="radio_{@name}" style="width:55" styleClass="clsButtonStyle"/></td>
			<td align="left"><db:deleteButton caption="Delete" associatedRadio="radio_{@name}" style="width:55" styleClass="clsButtonStyle"/></td>
		</tr>
	</table>
	<p/>
	<p/>
	<table border="0" align="center" width="400">
		<tr class="clsHeaderDataTableRow">
			<td colspan="2"><br/>I N S E R T new data ...<br/><br/></td>
		</tr>
		<xsl:for-each select="field">
		<!-- we create input fields for NON-AUTOMATIC values only -->
		...
		<tr>
			<td colspan="2">
				<center><hr/><db:insertButton caption="Insert new ..." styleClass="clsButtonStyle" showAlways="true"/></center>
			</td>
		</tr>
	</table>
	</db:footer>
