...
**
**   in contrast to "generate_list_editable" this XSL stylesheet also generates
**   navigation-buttons and it sets the maxCount attribute to "2" insteads of "*" 
**   [the value "2" can be changed to any other number, of course]
**  
...
<xsl:variable name="maxRows">2</xsl:variable>
<xsl:variable name="pageBgColor">99CCFF</xsl:variable>

<xsl:template match="table">

//--file "<xsl:value-of select="@name"/>_partial_list_editable.jsp" ------------------------------------------------
...
&lt;%int i=0; %&gt;
  ...
  <html>
	...
	<table bgcolor="#999900" cellpadding="1" cellspacing="0" width="100%" border="0" align="center">
     ...
     <db:dbform tableName="{@name}" maxRows="{$maxRows}" followUp="/{@name}_partial_list_editable.jsp" autoUpdate="false">
       <db:header>
	   ...
	   <td><db:updateButton caption="update" style="width:55"/></td>           
	   <td><db:deleteButton caption="delete" style="width:55"/></td>           	   
	   <xsl:text disable-output-escaping="yes">
           &lt;/tr&gt;
           </xsl:text>
